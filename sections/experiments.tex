...
\begin{figure}[!h]
\centering
    \makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{Plots and results/Confusion_Matrix_svm.pdf}}
...
\begin{figure}[!h]
\centering
    \makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{Plots and results/Confusion_Matrix_xgb.pdf}}
...
The best hyperparameters determined by the grid search in this case were $colsample \ bytree = 0.6$, $learning \ rate = 0.3$, and $max \ depth = 3$ for XGBoost and $C = 10.0$, $\gamma = 0.5$, $kernel = polynomial$ for SVM. It is evident that while the precision of SVM and XGBoost is relatively high for the "O" and "B-NEG" classes, it remains near zero for the "I-NEG" class on both Cardboard and Circle datasets. Regardless, macro-average precision and recall values demonstrate that the token feature holds some promise.
